...
const testDbPath = path.resolve(
  __dirname,
  '../../../test-fixtures/databases/testLanguageLearningDatabase.db'
);
...
describe('test fixture database', () => {
  ...
  it('has the minimal language/difficulty data', () => {
    const languages = query(
      'SELECT language FROM languages ORDER BY id;'
    ).split('\n');
    expect(languages).toEqual(['French', 'Spanish']);
    const difficulties = query(
      'SELECT difficulty_level FROM difficulties ORDER BY id;'
    ).split('\n');
  ...
  it('contains exercise rows for each type', () => {
    const counts = query(
      'SELECT exercise_type, COUNT(*) FROM exercises_info GROUP BY exercise_type ORDER BY exercise_type;'
    );
