...
function setupIPC() {
  const db = new LanguageDB("./src/database/languageLearningDatabase.db", function(err) {
    if (err) {
  ...
  ipcMain.on("add-user", (event, arg) => {
    ...
    ipcMain.on("get-topics-by-language-difficulty", (event, arg) => {
      db.getTopicsByLanguageDifficulty(arg.language, arg.difficulty, (err, topics) => {
        if (err) {
    ...
    ipcMain.on("get-random-pair-exercise", (event, arg) => {
      db.getRandomPairExercise(arg.language, arg.difficulty, arg.topic, (err, exercise) => {
        if (err) {
