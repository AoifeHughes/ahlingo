...
function Pairs({ onBack }) {
  ...
  useEffect(() => {
    if (selectedLanguage) {
      ipcRenderer.send("get-difficulty-by-language", {
        language: selectedLanguage,
      });
      ipcRenderer.on(
        "get-difficulty-by-language-response",
        (event, response) => {
  ...
  useEffect(() => {
    if (selectedLanguage && selectedDifficulty) {
      ...
      ipcRenderer.on(
        "get-topics-by-language-difficulty-response",
        (event, response) => {
  ...
  const handleTopicClick = (topic) => {
    ...
    ipcRenderer.send("get-random-pair-exercise", {
      language: selectedLanguage,
      difficulty: selectedDifficulty,
      topic,
    });
  ...
  const handleCellClick = (exerciseId, languageContent) => {
    ...
    if (
      ...
      (Object.keys(matches).length === 1 && !matches[exerciseId])
    ) {
    ...
    if (Object.keys(matches).length === 1) {
      ...
      if (
        exercises[firstId].language_2_content === languageContent ||
        exercises[firstId].language_1_content === languageContent
      ) {
          ...
          {shuffledExercises.map((exercise, index) => (
            <TableRow key={index} hover>
              <TableCell
                ...
                scope="row"
                onClick={() =>
                  handleCellClick(index, exercise[languageContentKey])
                }
                style={{
