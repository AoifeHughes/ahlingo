...
class LanguageDB {
  ...
  _initialize(callback) {
    ...
    addConversationExercise(exerciseName, language, topic, difficultyLevel, conversations, summary, callback) {
            ...
            this._getOrCreateDifficulty(difficultyLevel, (err, difficultyId) => {
              ...
              db.run(queryInsertExercise, [exerciseName, languageId, topicId, difficultyId], function(err) {
                ...
                conversations.forEach((conv, index) => {
                  db.run(insertConversationQuery, [exerciseId, index, conv.speaker, conv.message], (err) => {
                    if (err) return callback(err);
                    ...
                    if (index === conversations.length - 1) {
                      ...
                      db.run(insertSummaryQuery, [exerciseId, summary], (err) => {
