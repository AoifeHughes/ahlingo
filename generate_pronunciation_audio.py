...
class PronunciationAudioGenerator:
    ...
    def __init__(self, db_path: str, output_dir: str = None, reference_audio: str = None, speaker: str = None, ukr_voice: str = None):
        """
    ...
    def _generate_audio(self, text: str, language: str, output_path: Path = None, 
                       exercise_type: str = None, topic: str = None, difficulty: str = None) -> bool:
        """
        ...
        try:
            ...
            if lang_code == "uk" and UKRAINIAN_TTS_AVAILABLE:
                with open(temp_path, mode="wb") as file:
                    _, _ = self.ukrainian_tts.tts(text, self.ukr_voice, Stress.Dictionary.value, file)
            else:
                # Check if language is supported by XTTS-v2
                if lang_code not in self.supported_languages:
                    print(f"Warning: Language '{language}' (code: {lang_code}) not in supported languages list. Using English instead.")
                    lang_code = "en"
    ...
    def process_pair_exercises(self):
        ...
        for language in languages:
            ...
            for difficulty in difficulties:
                ...
                for topic in topics:
                    # Get pair exercises
                    exercises = self.db.get_random_pair_exercise(language, difficulty, topic, limit=1000)
                    
                    for exercise in tqdm(exercises, desc=f"Generating audio for {language} {difficulty} {topic} pairs"):
    ...
    def process_conversation_exercises(self):
        ...
        for conversation in tqdm(conversations, desc="Generating audio for conversations"):
            # Extract data
    ...
    def run(self):
        ...
        try:
            ...
            if self.output_dir:
                ...
                print(f"  - {len(self.metadata['conversations'])} conversation exercises")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate pronunciation audio files for language exercises")
    parser.add_argument("--db_path", default="./database/languageLearningDatabase.db", 
    ...
    parser.add_argument("--ukr_voice", default="Dmytro",
