...
def debug_show_error(prompt: str, response: str, error: str, context: str = "Validation"):
...
class DatabaseValidator:
    ...
    def validate_exercise(self, exercise_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            # Parse the result
            try:
                validation_result = parse_validation_result(cleaned_result_text, exercise_type)
                return validation_result
            except Exception as parse_error:
                # Show debug info for parsing errors
                exercise_id = exercise_data.get('id', 'unknown')
                debug_show_error(prepared_prompt, result_text, f"Validation Parse Error: {parse_error}", f"Validation (exercise_id={exercise_id})")
                raise parse_error
            ...
            if 'prepared_prompt' in locals() and 'result_text' in locals():
                debug_show_error(prepared_prompt, result_text, f"Validation Error: {e}", f"Validation (exercise_id={exercise_id})")
    ...
    def validate_exercise_batch(self, exercises_batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def validate_all_exercises(self, max_exercises: Optional[int] = None, exercise_type_filter: Optional[str] = None, 
                             max_workers: int = 5, batch_size: int = 50) -> Dict[str, Any]:
        """
        ...
        print(f"Validating {total_unvalidated} exercises with {max_workers} parallel workers...")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            with tqdm(total=total_unvalidated, desc="Database Batch Validation Progress") as pbar:
                ...
                for exercise_type in exercise_types:
                    ...
                    while total_processed < total_unvalidated:
                        ...
                        future = executor.submit(self.validate_exercise_batch, batch_exercises)
                            ...
                            with results_lock:
                                for result_data in batch_results:
                                    if 'failed_validation' in result_data:
                                        ...
                                        total_score += result_data['validation']['overall_quality_score']
...
def run_validation(
    ...
    if 'by_type' in counts:
        ...
        for ex_type, type_counts in counts['by_type'].items():
            print(f"  {ex_type}: {type_counts['unvalidated']} unvalidated / {type_counts['total']} total")
    
    # Run validation
    results = validator.validate_all_exercises(max_exercises, None, max_workers, batch_size)
