...
class FillInBlankExercise(BaseModel):
    ...
    def validate_sentence_blank(cls, v):
        ...
        if blank_count != 1:
            raise ValueError(f'Sentence must contain exactly one blank (_), found {blank_count}')
    ...
    def answer_not_empty(cls, v):
        ...
        if word_count > 3:
            raise ValueError(f'Answer options should be at most 3 words, got {word_count} words: "{v}"')
    ...
    def validate_blank_position(cls, v, values):
        ...
        if 'sentence' in values:
            ...
            if v >= estimated_word_count + 2:  # Allow some buffer
                raise ValueError(f'Blank position {v} seems out of range for sentence with ~{estimated_word_count} words')
