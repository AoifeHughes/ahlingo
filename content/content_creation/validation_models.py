...
class ValidationResult(BaseModel):
    ...
    is_correct_language: bool = Field(default=False, description="Is the text in the specified language? (true/false)")
    has_correct_grammar: bool = Field(default=False, description="Is the grammar correct? (true/false)")
    is_translation_accurate: bool = Field(default=False, description="Is the translation accurate? Use false for non-translation exercises (true/false)")
    is_culturally_appropriate: bool = Field(default=False, description="Is the content culturally appropriate? (true/false)")
    is_educational_quality: bool = Field(default=False, description="Is this of good educational quality for language learning? (true/false)")
    overall_quality_score: int = Field(default=1, description="Overall quality score from 1-10", ge=1, le=10)
...
class ConversationValidation(ValidationResult):
    ...
    has_natural_dialogue: bool = Field(default=False, description="Does the conversation flow naturally? (true/false)")
    appropriate_for_level: bool = Field(default=False, description="Is the difficulty appropriate for the specified level? (true/false)")


class PairValidation(ValidationResult):
    """Extended validation for word pair exercises."""
    translation_pairs_correct: bool = Field(default=False, description="Are all translation pairs accurate? (true/false)")
    appropriate_vocabulary_level: bool = Field(default=False, description="Is vocabulary appropriate for the level? (true/false)")


class TranslationValidation(ValidationResult):
    """Extended validation for translation exercises."""
    preserves_meaning: bool = Field(default=False, description="Does the translation preserve the original meaning? (true/false)")
    uses_natural_language: bool = Field(default=False, description="Does the translation use natural, idiomatic language? (true/false)")


class FillInBlankValidation(ValidationResult):
    """Extended validation for fill-in-blank exercises."""
    translation_matches_original: bool = Field(default=False, description="Does the English translation accurately convey the meaning of the original sentence? (true/false)")
    answer_options_appropriate: bool = Field(default=False, description="Are the answer options appropriate and at the right difficulty level? (true/false)")
    is_unambiguous: bool = Field(default=False, description="Is there only ONE clearly correct answer? No ambiguous situations where multiple options could work (true/false)")
...
def clean_validation_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    boolean_fields = [
        'is_correct_language', 'has_correct_grammar', 'is_translation_accurate',
        'is_culturally_appropriate', 'is_educational_quality', 'has_natural_dialogue',
        'appropriate_for_level', 'translation_pairs_correct', 'appropriate_vocabulary_level',
        'preserves_meaning', 'uses_natural_language', 'translation_matches_original',
        'answer_options_appropriate', 'is_unambiguous'
    ]
    ...
    if 'issues_found' in data:
        ...
        if isinstance(issues, list):
            cleaned['issues_found'] = [str(issue) for issue in issues if issue is not None]
...
def parse_validation_result(response_text: str, exercise_type: str) -> ValidationResult:
    ...
    try:
        ...
        print(f"Raw response: {response_text!r}")  # Show full response with repr for exact chars
